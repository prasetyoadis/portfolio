...
import { 
    getFirestore, collection, query, orderBy, onSnapshot, addDoc, serverTimestamp, doc, updateDoc, increment, getDoc, deleteField 
...
const q = query(
    ...
    orderBy("upVotes", "desc") // urut dari terbaru
);
            ...
            snapshot.forEach((doc) => {
                ...
                <div class="my-5 p-2 md:p-5 bg-white dark:bg-pasdev-surface-a00 shadow-md rounded-md">
                    ...
                    <div class="msg-action flex gap-1.5 mt-1">
                        <button id="up-${commentId}" data-comment-id="${commentId}" data-vote-type="up" class="btn-comment action ${myVote === 'up' ? 'active' : ''} " onclick="voteComment(this)">
                            ...
                            <span class="text-xs font-medium">${upVotes}</span>
                        </button>
                        <button id="down-${commentId}" data-comment-id="${commentId}" data-vote-type="down" class="btn-comment action ${myVote === 'down' ? 'active' : ''}" onclick="voteComment(this)">
                            <svg class="fill-current" xmlns="http://www.w3.org/2000/svg" width="16px" viewBox="0 -960 960 960">
                                <path d="M480-361q-8 0-15-2.5t-13-8.5L268-556q-11-11-11-28t11-28q11-11 28-11t28 11l156 156 156-156q11-11 28-11t28 11q11 11 11 28t-11 28L508-372q-6 6-13 8.5t-15 2.5Z"/>
                            </svg>
                            <span class="text-xs font-medium">${downVotes}</span>
                        </button>
            ...
            if (change.type === "added") {
                ...
                setTimeout(() => {
                    ...
                    <div class="my-5 p-2 md:p-5 bg-white dark:bg-pasdev-surface-a00 shadow-md rounded-md">
                        ...
                        <div class="msg-action flex gap-1.5 mt-1">
                            <button id="up-${commentId}" data-comment-id="${commentId}" data-vote-type="up" class="${myVote === 'up' ? 'btn-comment-active' : ''} flex items-center pl-0.5 py-1 pr-1.5 text-gray-400 dark:hover:text-white hover:text-black border border-transparent hover:border-gray-700 rounded-full cursor-pointer" onclick="voteComment(this)">
                                <svg class="fill-current" xmlns="http://www.w3.org/2000/svg" width="16px" viewBox="0 -960 960 960">
                                    <path d="M480-528 324-372q-11 11-28 11t-28-11q-11-11-11-28t11-28l184-184q12-12 28-12t28 12l184 184q11 11 11 28t-11 28q-11 11-28 11t-28-11L480-528Z"/>
                                </svg>
                                <span class="text-xs font-medium">${upVotes}</span>
                            </button>
                            <button id="down-${commentId}" data-comment-id="${commentId}" data-vote-type="down" class="${myVote === 'down' ? 'btn-comment-active' : ''}flex items-center pl-0.5 py-1 pr-1.5 text-gray-400 dark:hover:text-white hover:text-black border border-transparent hover:border-gray-700 rounded-full cursor-pointer" onclick="voteComment(this)">
                                <svg class="fill-current" xmlns="http://www.w3.org/2000/svg" width="16px" viewBox="0 -960 960 960">
                                    <path d="M480-361q-8 0-15-2.5t-13-8.5L268-556q-11-11-11-28t11-28q11-11 28-11t28 11l156 156 156-156q11-11 28-11t28 11q11 11 11 28t-11 28L508-372q-6 6-13 8.5t-15 2.5Z"/>
                                </svg>
                                <span class="text-xs font-medium">${downVotes}</span>
                ...
                const upEl = document.querySelector(`#up-${commentId}`);
                const downEl = document.querySelector(`#down-${commentId}`);
